...
export function editarPeliculaForm(pelicula) {
    let html = `<form action="/peliculas/editar/${pelicula.title}" method="POST" >`
    html += `
        <div class="mt-2" >
            <label class="form-label" >Titulo: </label>
        ...
        <div class="mt-2" >
            <label class="form-label" >Año de estreno: </label>
            <input class="form-control" type="number" name="year" value="${pelicula.year}"/>
        </div>
        <div class="mt-2" >
            <label class="form-label" >Genero: </label>
            <input class="form-control" type="text" name="primary_genre" value="${pelicula.primary_genre}"/>
        </div>   
       <div class="mt-2" >
            <label class="form-label" >Pais: </label>
            <input class="form-control" type="text" name="country" value="${pelicula.country}"/>
        </div>
        <div class="mt-2" >
            <label class="form-label" >Idioma: </label>
            <input class="form-control" type="text" name="language" value="${pelicula.language}"/>
        </div>
        <div class="mt-2" >
            <label class="form-label" >Produccion: </label>
            <input class="form-control" type="text" name="production_scale" value="${pelicula.production_scale}"/>
        </div>      
        <button class="btn btn-primary mt-2" type="submit" >Guardar</button>                 
    `
    html += `</form>`
    html += "<a href='/peliculas' class='mt-4' >Volver</a>"
    return createPage("Editar pelicula", html)
}

export function eliminarPeliculaForm(pelicula) {
    let html = ``
    html += `<form action="/peliculas/eliminar/${pelicula.title}" method="POST" >`
    html += `<p>Año de estreno: ${pelicula.year}</p>`
    html += `<p>Genero: ${pelicula.primary_genre}</p>`
    html += `<p>Pais: ${pelicula.country}</p>`
    html += `<p>Idioma: ${pelicula.language}</p>`
    html += `<p>Produccion: ${pelicula.production_scale}</p>`
