...
export async function movieExists(id){
    const db = conexion()
    console.log("id pelicula",id)
    const count = await db.collection("peliculas").countDocuments({_id:  new ObjectId(id)})
    return count > 0
}
...
export async function savePeliculaReview(id, usuario){
    ...
    const peliculaReview = await reviewService.saveReview(usuario, pelicula)
    return peliculaReview
}