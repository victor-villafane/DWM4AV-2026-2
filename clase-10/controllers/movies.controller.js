...
export async function editarPeliculaForm(req, res) {
    try {
        const title = req.params.title
        const pelicula = await movieService.getPeliculaByTitle(title)
        res.send(movieView.editarPeliculaForm(pelicula))
    } catch (error) {
        res.send(movieView.pageError("400", "No se pudo editar la pelicula"))
    }
}
...
export async function eliminarPeliculaForm(req, res) {
    try {
        const title = req.params.title
        const pelicula = await movieService.getPeliculaByTitle(title)
        res.send(movieView.eliminarPeliculaForm(pelicula))
    } catch (error) {
        res.send(movieView.pageError("400", "No se pudo borrar la pelicula"))
    }
}
...
export async function eliminarPelicula(req, res){
    try {
        const title = req.params.title
        const pelicula = await movieService.eliminarPeliculaLogico(title)
        res.send(movieView.createDetailPage(pelicula))
    } catch (error) {
        res.send(movieView.pageError("400", "No se pudo borrar la pelicula"))
    }
}